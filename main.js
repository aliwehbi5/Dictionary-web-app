...
function updateFontStyle(font) {
  ...
  fonts.forEach((f) => f.classList.remove("chosen"));
...
// Initialize font and mode
if (localStorage.getItem("font")) {
  let storedFont = JSON.parse(localStorage.getItem("font"));
  updateFontStyle(document.querySelector(`.${storedFont.className}`));
}
    ...
    if (meanings && meanings.length > 0) {
      meanings.forEach((meaning) => {
        ...
        if (antonym && antonym.length > 0) {
          ...
          document.querySelector(`.${partOfSpeech}`).appendChild(antonymDiv);
        }
        if (synonym && synonym.length > 0) {
          ...
          document.querySelector(`.${partOfSpeech}`).appendChild(synonymDiv);
        }
      });
    }
